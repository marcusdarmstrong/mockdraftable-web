...
const EmbededPlayer =
  ({ selectedPlayer, selectedPosition, percentiles, measurables, embedPage }: Props) => {
    const keyedMeasurements: { [MeasurableKey]: string } = selectedPlayer.measurements
      .reduce(
        (a, { measurableKey, measurement }) =>
          Object.assign({}, a, {
            [measurableKey]: format(measurement, measurables[measurableKey]),
          }),
        {},
      );
